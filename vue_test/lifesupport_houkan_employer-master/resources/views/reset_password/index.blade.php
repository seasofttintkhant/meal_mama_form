    ...
    <div>
		...
		<main class="main-with-sidebar pd-t-80">
			...
			<div class="card h-card h-p-30">
			  ...
			  @endif  
			  <form method="POST" action="{{ route('resetpassword.sendmail') }}">
			    {{ csrf_field() }}
			    <ul class="definition-table">
			        <li class="definition-table-item card-input-br-btm form-body">
			          <dl class="dl-definition-table">
			            <dt class="definition-table-head">
			              <span class="definition-head-item">
			              </span>
			              <span class="definition-head-item definition-head-item-left"><span>メールアドレス</span>
			              </span>
			            </dt>
			            <dd class="definition-table-body">
							{!! Form::text('email', null, ['placeholder'=>'example@houkanjob.com', 'class'=>'form-text-field']) !!}
			                @if ($errors->has('email'))
