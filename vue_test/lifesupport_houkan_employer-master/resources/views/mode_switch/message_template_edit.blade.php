...
<div class="content">
    ...
    <div class="card h-card h-p-30">
      ...
      <div class="space"></div>
        {!! Form::open(['route'=>['message-templates.update', $message_template->id],'method'=>'PATCH']) !!}

            {{ csrf_field() }}
            <div class="form-group">
                {!! Form::label('title','テンプレート名', ['class' => 'msg-label']) !!}
                {{ Form::text('title',$message_template->title,['class'=>'form-control']) }}
                @if ($errors->has('title'))
            ...
            <div class="form-group">
                ...
                {!! Form::textarea('content',$message_template->content,['size' => '30x5','class'=>'form-control msg-content','id'=>"description", 'max-length'=>3000]) !!}
